...
sweep_config = {
    ...
    "parameters": {
        "learning_rate": {"distribution": "uniform", "min": 0.0003, "max": 0.00005},
...
def main(config=None):
    ...
    learning_rate = config.learning_rate if config.learning_rate is not None else 0.0003
    n_steps = config.n_steps if config.n_steps is not None else 2048
    batch_size = config.batch_size if config.batch_size is not None else 64
    gamma = config.gamma if config.gamma is not None else 0.99

    env = OT2Env()
    env.reset()

    model = PPO("MlpPolicy", env, learning_rate=learning_rate, n_steps=n_steps, batch_size=batch_size, gamma=gamma, verbose=1, tensorboard_log="./logs_final_hpt")
